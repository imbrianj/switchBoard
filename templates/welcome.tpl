
      <div id="{{DEVICE_ID}}" class="device{{DEVICE_SELECTED}}">
        <div class="text-block">
          <h3>Welcome!</h3>
          <p>
            Welcome to the Universal Controller.  If you're seeing this message, it means you've got everything set up and you'll just need to configure your remote.
          </p>
          <p>
            Open up the config/config.js file which contains a series of standard device types.  Find any device that looks like one you'd like configured and plug in your relevant information.  For anything that's maybe not super obvious, there should be ample comments to help guide you.  While you're in there, you may also want to mark the "welcome" device as disabled: true to prevent this message from showing up anymore.
          </p>
          <p>
            Stuck?  Have questions?  Something go wrong or have any suggestions?  Feel free to <a href="https://github.com/imbrianj/universalController/issues/new" rel="external">file a ticket</a> or send me an email at <a href="mailto:brian@bevey.org" rel="external">brian@bevey.org</a>.
          </p>
        </div>
      </div>
