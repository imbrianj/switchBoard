...
exports.foscamControllerTest = {
  postPrepare : function (test) {
    ...
    var foscamController = require(__dirname + '/../../../controllers/foscam'),
        ...
        testUp           = foscamController.postPrepare({ deviceIp : 'TEST-host', devicePort: 80, username : 'TEST-username', password : 'TEST-password', command : 'Up' }),
        testTake         = foscamController.postPrepare({ deviceIp : 'TEST-host', devicePort: 80, username : 'TEST-username', password : 'TEST-password', command : 'Take' });
    ...
    test.deepEqual(testUp, { host : 'TEST-host', port : '80', path : '/decoder_control.cgi?user=TEST-username&pwd=TEST-password&command=0', method : 'GET' }, 'Additional params are filtered out.');

    test.deepEqual(testTake, { host : 'TEST-host', port : '80', path : '/snapshot.cgi?user=TEST-username&pwd=TEST-password&', method : 'GET' }, 'Additional params are filtered out.');
