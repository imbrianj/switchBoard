...
Each controller type is defined by many different components:
 ...
 - Inside the ```devices``` directory, you may create another subdirectory with the same name as you defined as your ```typeClass``` in the config above.  Within this new directory, you may create a ```controller.js``` file.  Within this file, the ```send()``` method will be executed for any command sent to this device.  Other functions may include:
